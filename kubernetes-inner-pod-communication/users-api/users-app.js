...
app.post('/signup', async (req, res) => {
  ...
  try {
    const hashedPW = await axios.get(`http://${process.env.AUTH_ADDR}/hashed-password/` + password);
    // since it's a dummy service, we don't really care for the hashed-pw either

    
    // const hashedPW = "texto pendejo";
...
app.post('/login', async (req, res) => {
  ...
  const response = await axios.get(
    `http://${process.env.AUTH_ADDR}/token/`+ hashedPassword + '/' + password
  );
